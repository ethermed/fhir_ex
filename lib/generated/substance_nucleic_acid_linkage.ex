defmodule Fhir.SubstanceNucleicAcidLinkage do
  ...
  embedded_schema do
    ...
    embeds_one(:_connectivity, Fhir.Element)
  ...
  def changeset(schema, params) do
    schema
    |> cast(params, [:connectivity, :id, :name, :residueSite])
    |> cast_embed(:_connectivity, with: &Fhir.Element.changeset/2)
    |> cast_embed(:_name, with: &Fhir.Element.changeset/2)
    |> cast_embed(:_residueSite, with: &Fhir.Element.changeset/2)
    |> cast_embed(:extension, with: &Fhir.Extension.changeset/2)
    |> cast_embed(:identifier, with: &Fhir.Identifier.changeset/2)
    |> cast_embed(:modifierExtension, with: &Fhir.Extension.changeset/2)
    |> validate_format(:connectivity, ~r/^^[\s\S]+$$/)
